...
public sealed class HybridVirtualMachine
{
    ...
    public IReadOnlyCollection<int> JittedFunctionIds { get; private set; } = Array.Empty<int>();

    public HybridVirtualMachine(BytecodeProgram program, RuntimeContext runtime, int hotThreshold = 50)
    {
        _program = program;
        _runtime = runtime;
